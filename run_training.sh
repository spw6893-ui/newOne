...
if [ "${1:-}" = "--help" ] || [ "${1:-}" = "-h" ]; then
    echo "用法:"
    echo "  ./run_training.sh [baseline|explore20|explore20_icir|explore20_faststable|explore20_lcb|explore20_ucblcb|explore20_ucblcb_fg]"
    echo ""
    echo "示例:"
...
case "$PRESET" in
    ...
    explore20_ucblcb)
        ...
        ;;
    explore20_ucblcb_fg)
        # 在 explore20_ucblcb 的基础上加“近似粗筛”提速：
        # - pool 未满：完全不影响训练动态
        # - pool 满后：先用小样本 single-IC 粗筛，明显减少 full mutual/optimize 的调用
        export_default ALPHAGEN_FEATURES_MAX 20
        export_default ALPHAGEN_FEATURES_PRUNE_CORR 0.95
        export_default ALPHAGEN_TOTAL_TIMESTEPS 800000
        ...
        # cache：尽量覆盖 pool + 候选，减少后期 eviction 导致的“越跑越慢”
        export_default ALPHAGEN_ALPHA_CACHE_SIZE 256

        # FastGate：只在 pool 满时启用（默认），阈值先给个温和值
